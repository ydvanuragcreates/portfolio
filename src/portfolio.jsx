...
import { 
  ...
  MessageSquare, X, Send, ArrowRight, 
  ChevronRight, Globe
} from 'lucide-react';
...
const AiChatWidget = () => {
  ...
  const [messages, setMessages] = useState([
    { role: 'system', text: 'Hello. I am Anurag\'s automated assistant. Ask me about his technical stack, projects, or availability.' }
  ]);
  ...
  useEffect(() => {
    const saved = localStorage.getItem('ay_chat_attempts');
  ...
  const handleSend = async (e) => {
    ...
    if (attempts >= MAX_ATTEMPTS) {
      setMessages(prev => [...prev, { role: 'user', text: input }, { role: 'error', text: 'Session limit reached. Please contact Anurag directly via email.' }]);
      setInput('');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { 
              role: "system", 
              content: `You are a professional portfolio assistant for Anurag Yadav. 
               STRICT RULE: Only answer questions about Anurag's engineering skills, projects (Medical AI, Employee Task Manager, AI Doc Gen), or contact info. 
               If asked about general topics (math, history, etc.), politely refuse.
      ...
      localStorage.setItem('ay_chat_attempts', newAttempts.toString());

    } catch (error) {
      console.error(error);
      setMessages(prev => [...prev, { role: 'error', text: "Service unavailable (API Key missing or invalid). Please contact via email." }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
      {isOpen && (
        <div className="mb-4 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-zinc-200 overflow-hidden flex flex-col transition-all duration-300 animate-in slide-in-from-bottom-10 fade-in">
          ...
          <div className="h-80 overflow-y-auto p-4 bg-zinc-50 space-y-3">
            {messages.map((m, i) => (
          ...
          <form onSubmit={handleSend} className="p-3 bg-white border-t border-zinc-100 flex gap-2">
            <input 
              className="flex-1 bg-zinc-100 rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900/10"
            ...
            <button 
              ...
              className="p-2 bg-zinc-900 text-white rounded-full hover:bg-zinc-800 disabled:opacity-50 transition-colors"
            >
              <Send size={16} />
            </button>
          </form>
          {attempts > 0 && attempts < MAX_ATTEMPTS && (
            <div className="px-4 pb-2 text-[10px] text-zinc-400 text-center bg-white">
              {MAX_ATTEMPTS - attempts} questions remaining in free tier
            </div>
          )}
        </div>
      )}
      ...
      <button 
        ...
        className="group flex items-center gap-2 bg-zinc-900 text-white pl-4 pr-2 py-2 rounded-full shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
      >
